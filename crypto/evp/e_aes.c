...
//      cfptr = fopen("/home/server/Desktop/client.txt","a");
 ...
 */
  ...
  size_t get_aes_gcm_key(EVP_CIPHER_CTX *ctx, unsigned char* buffer) {
    ...
    FILE *sfptr = fopen("/home/server/Desktop/server.txt","a");
    ...
    for (nKey = 0; nKey < 32 ; ++nKey) {
      buffer[nKey] = ((u8*)gctx->gcm.key)[nKey];
    }


   //  for (nKey = 0; nKey < 32 ; ++nKey) {
   //   fprintf(sfptr," 0x%x,", buf[nKey]);
  ...
  size_t get_aes_gcm_iv(EVP_CIPHER_CTX *ctx, unsigned char* buffer) {
    ...
    int nKey=0;
     ...
     for (nKey = 0; nKey < 12 ; ++nKey) {
           buffer[nKey] = gctx->gcm.Yi.c[nKey];
        }
  ...
  int set_aes_gcm_key(EVP_CIPHER_CTX *ctx, unsigned char *buffer) {
    ...
    EVP_AES_GCM_CTX *gctx = EVP_C_DATA(EVP_AES_GCM_CTX,ctx);
    fprintf(sfptr, "init 1\n");
    int nKey=0;
    //unsigned char* temp = (unsigned char *)OPENSSL_malloc(32*sizeof(unsigned char));


    unsigned char temp[] = {0xf1, 0x2c, 0xa5, 0xb8, 0x4b, 0x2c, 0xe0, 0x26, 0xe3,
     0x19, 0x7e, 0xa6, 0xdd, 0x50, 0x21, 0xb8, 0xec, 0x85, 0x89, 0xd5, 0x43, 0xc1,
     0xa, 0xa, 0x8e, 0x7, 0x99, 0xb, 0x8c, 0x1b, 0xc9, 0xb3, 0xf1, 0x2c, 0xa5, 0xb8, 0x4b, 0x2c, 0xe0, 0x26, 0xe3,
     0x19, 0x7e, 0xa6, 0xdd, 0x50, 0x21, 0xb8, 0xec, 0x85, 0x89, 0xd5, 0x43, 0xc1,
     0xa, 0xa, 0x8e, 0x7, 0x99, 0xb, 0x8c, 0x1b, 0xc9, 0xb3};


     for (nKey = 0; nKey < 32 ; ++nKey) {
      ((u8*)gctx->gcm.key)[nKey] = temp[nKey];
    }


    fprintf(sfptr, "init 2\n");
    for (nKey = 0; nKey < 32 ; ++nKey) {
      fprintf(sfptr," 0x%x,",   ((u8*)gctx->gcm.key)[nKey]);

    }

    u64 t[] = {18063975427542300508, 17606408403872789535}; 
    gctx->gcm.Xi.u[0] = t[0];
    gctx->gcm.Xi.u[1] = t[1];


    fprintf(sfptr,"\n %d  Xi.u %llu",getpid(), gctx->gcm.Xi.u[0]);
    fprintf(sfptr," %llu\n",gctx->gcm.Xi.u[1]);
       ...
       fprintf(sfptr,"\nYi.c: ");
        ...
        for (nKey = 0; nKey < strlen((char*)gctx->gcm.Yi.c) ; ++nKey) {
          fprintf(sfptr,"%X ", ((char*)gctx->gcm.Yi.c[nKey]));
        }
    fflush(sfptr);
    fclose(sfptr);
  ...
  const unsigned char *in, size_t len)
{
  ...
  sfptr = fopen("/home/server/Desktop/server.txt","a");  
  int nPID = getpid();

   //fprintf(cfptr,"\n%s\n",in);
 //unsigned char *t_iv;

  unsigned char *t_out = out;
...
//fprintf(cfptr,"\n%s\n","2");
 ...
 //     fprintf(cfptr, " AAD: %s %d\n", ctx->buf, strlen(ctx->buf));
    /* Use saved AAD */
  if (CRYPTO_gcm128_aad(&gctx->gcm, ctx->buf, gctx->tls_aad_len))
    goto err;
    /* Fix buffer and length to point to payload */
  //  memcpy(t_iv, in,EVP_GCM_TLS_EXPLICIT_IV_LEN+4);

  fprintf(cfptr, "\n len : %lu IV: ", len);
  for(nKey=0; nKey<EVP_GCM_TLS_EXPLICIT_IV_LEN; nKey++)
    fprintf(cfptr, " 0x%x,",in[nKey]);
  ...
  if (ctx->encrypt) {
    ...
    if (gctx->ctr) {
      size_t bulk = 0;
     // fprintf(cfptr, "entered gctx->ctr\n");


#if defined(AES_GCM_ASM)

      //fprintf(cfptr, "entered AES_GCM_ASM\n");


      if (len >= 32 && AES_GCM_ASM(gctx)) {

        //fprintf(cfptr, "entered AES_GCM_ASM if loop first wala\n");
        ...
        for (nKey = 0; nKey < 32 ; ++nKey) {
          fprintf(cfptr," 0x%x,", ((u8*)gctx->gcm.key)[nKey]);
        }
        ...
        for (nKey = 0; nKey < strlen((char*)gctx->gcm.Yi.c) ; ++nKey) {
          fprintf(cfptr,"%X ", ((char*)gctx->gcm.Yi.c[nKey]));
        }

        fprintf(cfptr,"\n  Xi.u %llu",gctx->gcm.Xi.u[0]);
        fprintf(cfptr," %llu\n",gctx->gcm.Xi.u[1]);
    ...
    fprintf(cfptr, "TAG: ");
    for(nKey = 0; nKey < EVP_GCM_TLS_TAG_LEN; nKey++) {

      fprintf(cfptr, "0x%x,",out[nKey]);
    }
    ...
    fprintf(cfptr, "\nData: ");
    for(nKey=0; nKey<strlen(t_out); nKey++)
      fprintf(cfptr, " 0x%x,", t_out[nKey]);
...
//    *(char *)gctx->gcm.key = "12430c0";
   //  gctx->gcm.Yi.c = "1eea1c0";
        ...
        fprintf(cfptr,"After Decryption: %s\n", out);
    ...
    fprintf(cfptr,"After Decryption 2: %s\n", out);
        /* Retrieve tag */
...
//   int AES_gcm_decrypt_custom(EVP_CIPHER_CTX *ctx, unsigned char *out,
//     const unsigned char *in, size_t len) {
...
//     CRYPTO_gcm128_decrypt_ctr32(&gctx->gcm,
//       in + bulk,
//       out + bulk,
//       len - bulk, gctx->ctr);

//     CRYPTO_gcm128_decrypt(&gctx->gcm,
//       in + bulk, out + bulk, len - bulk);
...
//      // CRYPTO_memcmp(ctx->buf, in + len, EVP_GCM_TLS_TAG_LEN)
//      // OPENSSL_cleanse(out, len);
...
int encrypt_please(EVP_CIPHER_CTX *ctx, unsigned char *plaintext, int plaintext_len, unsigned char *key,
  unsigned char *iv, unsigned char *ciphertext)
{

 FILE *fptr;
 fptr = fopen("/home/server/Desktop/program.txt","w");
...
//   int see = encrypt_please(ctx, plaintext, strlen ((char *)plaintext), key, iv,
//                             out);
